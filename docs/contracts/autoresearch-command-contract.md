# `omcp autoresearch` parity contract

`omcp autoresearch` is a thin supervisor that drives one Codex experiment session per iteration while OMCP owns the durable keep/discard/reset loop.

## CLI

```bash
omcp autoresearch <mission-dir> [codex-args...]
omcp autoresearch --resume <run-id> [codex-args...]
omcp autoresearch --help
```

- Fresh launch always creates a new run-tagged lane.
- `--resume <run-id>` loads `.omcp/logs/autoresearch/<run-id>/manifest.json`.
- A second launch is rejected while repo-root `.omcp/state/autoresearch-state.json` points at an active run.
...
- worktree `<repo>.omcp-worktrees/autoresearch-<mission-slug>-<run-tag>`
- repo-root run artifacts under `.omcp/logs/autoresearch/<run-id>/`

Repo-root state responsibilities:
- `.omcp/state/autoresearch-state.json` = active-run pointer/lock only
- `.omcp/logs/autoresearch/<run-id>/manifest.json` = authoritative per-run state
- `.omcp/logs/autoresearch/<run-id>/candidate.json` = candidate handoff from the just-finished Codex session
- `.omcp/logs/autoresearch/<run-id>/iteration-ledger.json` = durable iteration history
- `.omcp/logs/autoresearch/<run-id>/latest-evaluator-result.json` = latest evaluator output

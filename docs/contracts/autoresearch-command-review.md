# `omcp autoresearch` Full-Parity Review Notes
...
- `.omcp/plans/prd-autoresearch-full-parity.md`
- `.omcp/plans/test-spec-autoresearch-full-parity.md`
...
- `omcp autoresearch` exposes both fresh launch and `--resume <run-id>` flows.
...
**Pass.**
- repo-root active-run lock lives at `.omcp/state/autoresearch-state.json`.
- per-run manifest, candidate, ledger, and latest evaluator files live under `.omcp/logs/autoresearch/<run-id>/`.

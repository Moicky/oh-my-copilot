...
|---|---|---|
| `omcp team status` | `.omcp/state/team/<team>/config.json`, `manifest.v2.json`, `tasks/*.json`, `approvals/*.json`, `workers/*` | Manifest-backed team config is authoritative when both config and manifest exist. |
| `omcp doctor --team` | `.omcp/state/team/<team>/config.json`, `manifest.v2.json`, `workers/*/status.json`, `workers/*/heartbeat.json`, `.omcp/state/hud-state.json` | Manifest-backed tmux/session identity is authoritative when both config and manifest exist. |
| HUD readers | `.omcp/state/session.json`, `.omcp/state/sessions/<session>/team-state.json`, `.omcp/state/team-state.json`, `.omcp/state/ralph-state.json` | Session-scoped files are authoritative when a session is active; root files are compatibility fallback only. |

## Rust-authored runtime files

The `RuntimeEngine` (in `crates/omcp-runtime-core/src/engine.rs`) writes the following files via `persist()` and `write_compatibility_view()`:

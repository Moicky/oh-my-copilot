# `omcp autoresearch` deep-interview UX review notes
...
- `.omcp/plans/prd-autoresearch-ux-deep-interview.md`
- `.omcp/plans/test-spec-autoresearch-ux-deep-interview.md`
...
- `omcp autoresearch` with no args enters `guidedAutoresearchSetup()` and immediately asks for a raw evaluator command.
- bare `init` still routes into that same guided path instead of an explicit novice compatibility bridge.
- there is no autoresearch-specific intake module, no canonical `.omcp/specs/deep-interview-autoresearch-{slug}.md` artifact, and no confirm/refine launch gate.
...
- `src/cli/autoresearch-guided.ts`
  ...
  - no draft artifact is written under `.omcp/specs/`.
- `skills/deep-interview/SKILL.md`
...
- `src/cli/__tests__/autoresearch.test.ts`
  - still asserts the older `omcp autoresearch init [--topic ...]` help surface and non-interactive `mission-dir is required` failure.
...
### 3. Canonical draft artifact at `.omcp/specs/deep-interview-autoresearch-{slug}.md`
**Fail on current branch.**
No `.omcp/specs/` draft artifact is generated.
...
### 8. Bare `omcp autoresearch init` documented as novice alias
**Fail on current branch.**
...
1. Update `README.md` with the new novice entry surfaces:
   - `omcp autoresearch` as a deep-interview-style refinement flow

...
OMCP emits hook events for clawhip through the existing hooks extensibility pipeline.

## Canonical routing rule

Route on `context.normalized_event`, not just raw `event`.

This keeps clawhip stable even when OMCP uses legacy-compatible raw event names such as `session-start`, `session-end`, and `session-idle`.
...
When available, OMCP includes these fields in `context`:
...
- operational contexts resolve `session_name` from the OMCP session id + worktree so session metadata stays stable across native and derived events

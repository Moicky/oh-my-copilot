...
| CLI help smoke | `node bin/omcp.js --help` | PASS (`real 0.10`) |
| Version smoke | `node bin/omcp.js version` | PASS (`oh-my-copilot v0.8.3`, `real 0.10`) |
| Status smoke | `node bin/omcp.js status` | PASS (`ultrawork: ACTIVE (phase: debugging-verification)`, `real 0.13`) |
| Doctor smoke | `node bin/omcp.js doctor` | PASS (`9 passed, 0 warnings, 0 failed`, `real 0.21`) |
| Setup dry-run smoke | `node bin/omcp.js setup --dry-run` | PASS (`real 0.58`) |
| Cancel smoke | `node bin/omcp.js cancel` | PASS (`Cancelled: ultrawork`) |

...
- expected `instructions-path: .../.omcp/state/sessions/omcp-*/AGENTS.md`
- actual `instructions-path: .../.omcp/team/continue-from-clean-commit-810/worktrees/worker-3/AGENTS.md`

Interpretation:

- The test expects the `omcp exec` path to always use a generated session-scoped overlay file.
...
```text
expected: /instructions-path:.*\/\.omcp\/state\/sessions\/omcp-.*\/AGENTS\.md/
actual: fake-codex:exec --model gpt-5 say hi -c model_instructions_file="/home/.../.omcp/team/continue-from-clean-commit-810/worktrees/worker-3/AGENTS.md"
```
...
- The full-suite evidence for this snapshot is stored in `.omcp/context/task-3-npm-test-20260319.log`.
- Focused repro logs were stored in:
  - `.omcp/context/task-3-exec-test.log`
  - `.omcp/context/task-3-codebase-map-test.log`

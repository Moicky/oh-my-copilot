# Heavy / Manual Stress Lane: `omcp-explore` and `omcp-sparkshell`
...
| Command | Result |
|---|---|
| `node bin/omcp.js explore --help` | PASS |
| `node bin/omcp.js sparkshell --help` | PASS |
| `node bin/omcp.js sparkshell git --version` | PASS |
| `node bin/omcp.js version` | PASS |
| `for i in $(seq 1 20); do node bin/omcp.js sparkshell git --version; done` | PASS (bounded sample verified locally) |
| `seq 1 5 | xargs -I{} -P5 node bin/omcp.js sparkshell git --version` | PASS (bounded sample verified locally) |
...
**Goal**
- Confirm `omcp-sparkshell --tmux-pane` preserves operator-critical facts when pane tails are large and noisy.
...
```bash
node bin/omcp.js sparkshell git --version
```

Suggested repeated-run shape:

```bash
for i in $(seq 1 20); do node bin/omcp.js sparkshell git --version; done
```

Suggested concurrent-run shape:

```bash
seq 1 5 | xargs -I{} -P5 node bin/omcp.js sparkshell git --version
```
...
**Goal**
- Stress `omcp-explore` and `omcp-sparkshell` summarization with outputs that bury signal under distractors.
...
```bash
node bin/omcp.js sparkshell --help
node bin/omcp.js explore --help
```

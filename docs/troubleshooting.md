...
Use this page when OMCP appears installed but real Codex execution still fails.
...
1. Print `HOME` and `CODEX_HOME` in the shell that launches OMCP.
...
This is usually **not** a net-new OMCP feature gap.

OMCP already carries the tmux-side preservation work from issue `#1271` / PR `#1273` (`4405f582`, “Preserve Shift+Enter inside tmux-backed OMCP launches”), and current `dev` still enables tmux `extended-keys=always` around OMCP-owned Codex launch paths:
...
2. **the reporter is not in the OMCP-owned tmux launch path**
   - for example, reproducing in a different pane/session than the one OMCP launched or after attaching through a different client path
...
Expected first check: `always` while OMCP is actively running Codex in that tmux-managed path.

If `extended-keys` is **not** `always` during the failing session, that points to an OMCP launch-path bug/regression.

If `extended-keys` **is** `always`, but `Shift+Enter` still submits, the likely problem is terminal capability discovery or upstream Codex terminal-input interpretation rather than OMCP submission logic.
...
- **Open a code fix** only if you can show current `dev` fails to set `extended-keys=always` on the live OMCP-owned tmux launch path.
...
- **Prefer a docs follow-up** when the root problem is discoverability/operator guidance rather than a broken OMCP codepath.

...
`omx adapt <target>` is the OMCP-owned surface for persistent external-agent adaptation.
...
- `omx adapt openclaw envelope` includes lifecycle bridge metadata for the existing OMCP to OpenClaw event mapping

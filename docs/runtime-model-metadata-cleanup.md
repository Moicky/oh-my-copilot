...
After rebuilding and rerunning `omcp setup --force`, spawned-agent metadata should stop surfacing `haiku` / `sonnet` / `opus` as if they were active runtime models for Codex-based OMCP runs.

## Re-test steps

```bash
npm run build   # TypeScript build
node bin/omcp.js setup --scope project --force
```

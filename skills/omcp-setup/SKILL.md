...
```bash
omcp setup [--force] [--dry-run] [--verbose] [--scope <user|project>]
```

If you only want lightweight `AGENTS.md` scaffolding for an existing repo or subtree, use `omcp agents-init [path]` instead of full setup.
...
`omcp setup` performs these steps:

1. Resolve setup scope:
   - `--scope` explicit value
   - else persisted `./.omcp/setup-scope.json` (with automatic migration of legacy values)
...
6. Configure notify hook references and write `./.omcp/hud-config.json`

## Important behavior notes

- `omcp setup` only prompts for scope when no scope is provided/persisted and stdin/stdout are TTY.
...
- If your global `omcp` points to another install, run local entrypoint:

```bash
node bin/omcp.js setup --force --verbose
node bin/omcp.js doctor
```

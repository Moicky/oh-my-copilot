...
- **After Pass 1 completes**: Write extraction summary to `.omcp/state/{scope}/web-clone-extraction.json` containing:
...
- **After each Pass 4 verification**: Append the composite verdict to `.omcp/state/{scope}/web-clone-verdicts.json`.
- **When running within ralph**: Also persist the `visual` portion of the composite verdict to `.omcp/state/{scope}/ralph-progress.json` for ralph compatibility, mapping `visual.score` → top-level `score` and `visual.verdict` → top-level `verdict`.

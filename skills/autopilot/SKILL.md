...
0. **Pre-context Intake (required before Phase 0 starts)**:
   ...
   - Load the latest relevant snapshot from `.omcp/context/{slug}-*.md` when available.
   - If no snapshot exists, create `.omcp/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
     - Task statement
     - Desired outcome
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - If `.omcp/specs/deep-interview-*.md` exists for this task: reuse it and skip redundant expansion work
   ...
   - Output: `.omcp/plans/autopilot-spec.md`

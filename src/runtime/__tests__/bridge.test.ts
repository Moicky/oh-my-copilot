...
describe('resolveRuntimeBinaryPath', () => {
  it('prefers explicit OMCP_RUNTIME_BINARY override', () => {
    const previous = process.env.OMCP_RUNTIME_BINARY;
    try {
      process.env.OMCP_RUNTIME_BINARY = '/custom/runtime';
      ...
      if (typeof previous === 'string') process.env.OMCP_RUNTIME_BINARY = previous;

...
Available showcases:
  omcp-self       -> missions/in-action-cat-shellout-demo
  ...
  all            -> omcp-self ml-tabular bayesopt latent sorting

Examples:
  scripts/run-autoresearch-showcase.sh --list
  scripts/run-autoresearch-showcase.sh bayesopt
  scripts/run-autoresearch-showcase.sh omcp-self ml-tabular
USAGE
}

mission_for() {
  case "$1" in
    omcp-self)   printf '%s' 'missions/in-action-cat-shellout-demo' ;;
...
args=("$@")
if [[ " ${args[*]} " == *" all "* ]]; then
  args=(omcp-self ml-tabular bayesopt latent sorting)
fi

 ...
 * owned path.
 * Disable entirely with OMCP_RUNTIME_BRIDGE=0.
 */
function runtimeExec(command, stateDir) {
  if (process.env.OMCP_RUNTIME_BRIDGE === '0') return;
  try {
...
const ISSUE_DISPATCH_COOLDOWN_ENV = 'OMCP_TEAM_DISPATCH_ISSUE_COOLDOWN_MS';
const DEFAULT_DISPATCH_TRIGGER_COOLDOWN_MS = 30 * 1000;
const DISPATCH_TRIGGER_COOLDOWN_ENV = 'OMCP_TEAM_DISPATCH_TRIGGER_COOLDOWN_MS';
...
export async function drainPendingTeamDispatch({
  ...
  if (safeString(process.env.OMCP_TEAM_WORKER)) {

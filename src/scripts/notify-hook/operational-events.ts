...
function buildTmuxSessionName(cwd: any, sessionId: any): string {
  ...
  const dirToken = parentDir.endsWith('.omcp-worktrees')
    ? sanitizeTmuxToken(`${parentDir.slice(0, -'.omcp-worktrees'.length)}-${dirName}`)
  ...
  const sessionToken = sanitizeTmuxToken(safeString(sessionId).replace(/^omcp-/, ''));

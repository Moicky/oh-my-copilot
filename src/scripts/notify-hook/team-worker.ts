...
export function resolveWorkerIdleNotifyEnabled() {
  const raw = safeString(process.env.OMCP_TEAM_WORKER_IDLE_NOTIFY || '').trim().toLowerCase();
...
export function resolveWorkerIdleCooldownMs() {
  const raw = safeString(process.env.OMCP_TEAM_WORKER_IDLE_COOLDOWN_MS || '');
...
export function resolveAllWorkersIdleCooldownMs() {
  const raw = safeString(process.env.OMCP_TEAM_ALL_IDLE_COOLDOWN_MS || '');
  const parsed = asNumber(raw);
...
export function resolveStatusStaleMs() {
  const raw = safeString(process.env.OMCP_TEAM_STATUS_STALE_MS || '');
  const parsed = asNumber(raw);
...
export function resolveHeartbeatStaleMs() {
  const raw = safeString(process.env.OMCP_TEAM_HEARTBEAT_STALE_MS || '');
  const parsed = asNumber(raw);
  if (parsed !== null && parsed >= 5_000 && parsed <= 60 * 60_000) return parsed;
  return 180_000;
}

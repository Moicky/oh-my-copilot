...
const binaryName = process.platform === 'win32' ? 'omcp-explore-harness.exe' : 'omcp-explore-harness';
const sourcePath = join(root, 'target', 'release', binaryName);
const outputPath = join(binDir, binaryName);
const metadataPath = join(binDir, 'omcp-explore-harness.meta.json');

const build = spawnSync('cargo', ['build', '--release', '-p', 'omcp-explore-harness'], {
  cwd: root,
  encoding: 'utf-8',

 ...
 *
 * File location:
 *   With session id : .omcp/state/sessions/<session_id>/prompt-routing-state.json
 *   Without session id: .omcp/state/prompt-routing-state.json
 *
 * Rules:
...
function resolveStatePath(workingDirectory?: string, sessionId?: string | null): string | null {
  ...
  // getStateDir returns .omcp/state/sessions/<sessionId>/ when sessionId is provided,
  // or .omcp/state/ as the root fallback — exactly the paths we need.

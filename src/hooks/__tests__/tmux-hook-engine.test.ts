...
describe('normalizeTmuxHookConfig', () => {
  ...
  it('normalizes valid config', () => {
    const config = normalizeTmuxHookConfig({
      enabled: true,
      target: { type: 'session', value: 'omcp-work' },
      allowed_modes: ['team'],
      cooldown_ms: 2000,
    ...
    assert.deepEqual(config.target, { type: 'session', value: 'omcp-work' });
...
describe('evaluateInjectionGuards', () => {
  const validConfig = normalizeTmuxHookConfig({
    enabled: true,
    target: { type: 'session', value: 'omcp' },
    allowed_modes: ['ralph'],
    cooldown_ms: 1000,
...
describe('resolveCodexPane', () => {
  it('ignores HUD pane even when TMUX_PANE foreground command is node', async () => {
    ...
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omcp-resolve-codex-pane-'));
...
if [[ "$cmd" == "display-message" ]]; then
  ...
  if [[ "$format" == "#{pane_start_command}" && "$target" == "%2" ]]; then
    echo "node /pkg/dist/cli/omcp.js hud --watch"
    exit 0
  fi

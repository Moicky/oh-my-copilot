...
describe('createHookPluginSdk', () => {
  ...
  describe('tmux.sendKeys', () => {
    ...
    it('prefers non-HUD codex pane when targeting a tmux session', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omcp-sdk-'));
      const fakeBinDir = await mkdtemp(join(tmpdir(), 'omcp-sdk-bin-'));
...
`);
  ...
  describe('omcp', () => {
    it('exposes only the explicit read-only omcp readers', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omcp-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });

        assert.deepEqual(Object.keys(sdk.omcp).sort(), ['hud', 'notifyFallback', 'session', 'updateCheck']);
        assert.equal(typeof sdk.omcp.session.read, 'function');
        assert.equal(typeof sdk.omcp.hud.read, 'function');
        assert.equal(typeof sdk.omcp.notifyFallback.read, 'function');
        assert.equal(typeof sdk.omcp.updateCheck.read, 'function');
        assert.equal('pluginState' in sdk, false);
        assert.equal('readJson' in sdk.omcp, false);
        assert.equal('list' in sdk.omcp, false);
        assert.equal('exists' in sdk.omcp, false);
        assert.equal('write' in sdk.omcp.session, false);
        assert.equal('delete' in sdk.omcp.session, false);
        assert.equal('write' in sdk.omcp.hud, false);
        assert.equal('delete' in sdk.omcp.hud, false);
        assert.equal('write' in sdk.omcp.notifyFallback, false);
        assert.equal('delete' in sdk.omcp.notifyFallback, false);
        assert.equal('write' in sdk.omcp.updateCheck, false);
        assert.equal('delete' in sdk.omcp.updateCheck, false);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('reads session state from .omcp/state/session.json', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        const state = await sdk.omcp.session.read();
    ...
    it('returns null for invalid session state without session_id', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omcp.session.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('reads hud, notifyFallback, and updateCheck state from root-scoped omcp files', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.deepEqual(await sdk.omcp.hud.read(), {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
        });
        assert.deepEqual(await sdk.omcp.notifyFallback.read(), {
          pid: 1234,
          stopping: false,
          tracked_files: 2,
        });
        assert.deepEqual(await sdk.omcp.updateCheck.read(), {
          last_checked_at: '2026-01-01T00:00:00.000Z',
          last_seen_latest: '0.11.0',
        });
      } finally {
    ...
    it('returns null for missing omcp reader files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omcp-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omcp.session.read(), null);
        assert.equal(await sdk.omcp.hud.read(), null);
        assert.equal(await sdk.omcp.notifyFallback.read(), null);
        assert.equal(await sdk.omcp.updateCheck.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
...
describe('clearHookPluginState', () => {
  it('removes data.json and tmux.json for plugin', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-clear-'));
    try {
      const pluginDir = join(cwd, '.omcp', 'state', 'hooks', 'plugins', 'my-plugin');

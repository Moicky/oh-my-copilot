...
describe('isHookPluginFeatureEnabled', () => {
  it('returns true when OMCP_HOOK_PLUGINS=1', () => {
    assert.equal(isHookPluginFeatureEnabled({ OMCP_HOOK_PLUGINS: '1' }), true);
  });
  ...
  it('returns false for "0"', () => {
    assert.equal(isHookPluginFeatureEnabled({ OMCP_HOOK_PLUGINS: '0' }), false);
  });
});
...
describe('dispatchHookEvent', () => {
  ...
  it('dispatches valid plugins successfully', async () => {
    ...
    try {
      ...
      const result = await dispatchHookEvent(event, {
        cwd,
        env: { ...process.env, OMCP_HOOK_PLUGINS: '1' },
      });
...
`,
  ...
  it('disables side effects for team workers by default', async () => {
    ...
    try {
      ...
      const result = await dispatchHookEvent(event, {
        cwd,
        env: { ...process.env, OMCP_HOOK_PLUGINS: '1', OMCP_TEAM_WORKER: 'worker-1' },
      });

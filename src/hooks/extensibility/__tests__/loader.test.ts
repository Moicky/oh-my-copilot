...
describe('hooksDir', () => {
  it('returns .omcp/hooks under cwd', () => {
    assert.equal(hooksDir('/project'), join('/project', '.omcp', 'hooks'));
  });
});
...
describe('ensureHooksDir', () => {
  it('creates .omcp/hooks directory and returns path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ensure-'));
    try {
      const dir = await ensureHooksDir(cwd);
      assert.equal(dir, join(cwd, '.omcp', 'hooks'));
...
describe('discoverHookPlugins', () => {
  it('returns empty array when hooks directory does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-discover-'));
    try {
...
describe('validateHookPluginExport', () => {
  it('returns valid for plugin with onHookEvent export', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-validate-'));
    try {
...
describe('loadHookPluginDescriptors', () => {
  it('returns empty array when no hooks directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-load-'));
    try {

...
describe('mcp/code-intel-server module contract', () => {
  ...
  it('skips tsc diagnostics when the project has no tsconfig', async () => {
    const previous = process.env.OMCP_CODE_INTEL_SERVER_DISABLE_AUTO_START;
    process.env.OMCP_CODE_INTEL_SERVER_DISABLE_AUTO_START = '1';
    ...
    try {
      ...
      if (previous === undefined) {
        delete process.env.OMCP_CODE_INTEL_SERVER_DISABLE_AUTO_START;
      } else {
        process.env.OMCP_CODE_INTEL_SERVER_DISABLE_AUTO_START = previous;
      }

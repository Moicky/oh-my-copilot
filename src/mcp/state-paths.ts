...
const WORKDIR_ALLOWLIST_ENV = 'OMCP_MCP_WORKDIR_ROOTS';
...
export function getBaseStateDir(workingDirectory?: string): string {
  if ((workingDirectory == null || workingDirectory === '') && typeof process.env.OMCP_TEAM_STATE_ROOT === 'string' && process.env.OMCP_TEAM_STATE_ROOT.trim() !== '') {
    try {
      return resolveWorkingDirectoryForState(process.env.OMCP_TEAM_STATE_ROOT.trim());
    } catch {}
  }

...
describe('modes/base multi-state compatibility', () => {
  it('allows the approved team + ralph overlap across root and session scopes', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-mode-team-ralph-'));
    try {
      ...
      assert.equal(existsSync(join(wd, '.omcp', 'state', 'team-state.json')), true);
      assert.equal(
        existsSync(join(wd, '.omcp', 'state', 'sessions', 'sess-team-ralph', 'ralph-state.json')),
        true,
      );
  ...
  it('rejects standalone autopilot + team overlaps with actionable clearing guidance', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-mode-autopilot-team-'));
    try {
      ...
      const autopilotState = JSON.parse(
        await readFile(join(wd, '.omcp', 'state', 'autopilot-state.json'), 'utf-8'),
      ) as { active?: boolean };
      assert.equal(autopilotState.active, true);
      assert.equal(existsSync(join(wd, '.omcp', 'state', 'team-state.json')), false);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

...
const originalFrontierEnv = process.env.OMCP_DEFAULT_FRONTIER_MODEL;
const originalStandardEnv = process.env.OMCP_DEFAULT_STANDARD_MODEL;
const originalSparkEnv = process.env.OMCP_DEFAULT_SPARK_MODEL;
const originalLegacySparkEnv = process.env.OMCP_SPARK_MODEL;
...
afterEach(() => {
  if (typeof originalFrontierEnv === 'string') {
    process.env.OMCP_DEFAULT_FRONTIER_MODEL = originalFrontierEnv;
  } else {
    delete process.env.OMCP_DEFAULT_FRONTIER_MODEL;
  }
  if (typeof originalStandardEnv === 'string') {
    process.env.OMCP_DEFAULT_STANDARD_MODEL = originalStandardEnv;
  } else {
    delete process.env.OMCP_DEFAULT_STANDARD_MODEL;
  }
  if (typeof originalSparkEnv === 'string') {
    process.env.OMCP_DEFAULT_SPARK_MODEL = originalSparkEnv;
  } else {
    delete process.env.OMCP_DEFAULT_SPARK_MODEL;
  }
  if (typeof originalLegacySparkEnv === 'string') {
    process.env.OMCP_SPARK_MODEL = originalLegacySparkEnv;
  } else {
    delete process.env.OMCP_SPARK_MODEL;
  }
});
...
describe('agents model table', () => {
  it('resolves frontier from config.toml, standard from environment, and spark from environment', () => {
    process.env.OMCP_DEFAULT_FRONTIER_MODEL = 'frontier-env';
    process.env.OMCP_DEFAULT_STANDARD_MODEL = 'standard-env';
    process.env.OMCP_DEFAULT_SPARK_MODEL = 'spark-env';

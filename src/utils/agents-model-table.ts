...
export const OMCP_MODELS_START_MARKER = '<!-- OMCP:MODELS:START -->';
export const OMCP_MODELS_END_MARKER = '<!-- OMCP:MODELS:END -->';
...
export function buildAgentsModelTable(
  ...
  return [
    ...
    '',
    'Auto-generated by `omcp setup` from the current `config.toml` plus OMCP model overrides.',
    '',
...
export function upsertAgentsModelTable(
  ...
  const startIndex = content.indexOf(OMCP_MODELS_START_MARKER);
  const endIndex = content.indexOf(OMCP_MODELS_END_MARKER);

  if (startIndex >= 0 && endIndex > startIndex) {
    const replaceEnd = endIndex + OMCP_MODELS_END_MARKER.length;

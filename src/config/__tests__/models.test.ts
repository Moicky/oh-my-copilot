...
describe('getModelForMode', () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.OMCP_SPARK_MODEL;
  });
  ...
  afterEach(async () => {
    ...
    if (typeof originalDefaultStandardModel === 'string') {
      process.env.OMCP_DEFAULT_STANDARD_MODEL = originalDefaultStandardModel;
    } else {
      delete process.env.OMCP_DEFAULT_STANDARD_MODEL;
    }
    if (typeof originalDefaultSparkModel === 'string') {
      process.env.OMCP_DEFAULT_SPARK_MODEL = originalDefaultSparkModel;
    } else {
      delete process.env.OMCP_DEFAULT_SPARK_MODEL;
    }
    if (typeof originalSparkModel === 'string') {
      process.env.OMCP_SPARK_MODEL = originalSparkModel;
    } else {
      delete process.env.OMCP_SPARK_MODEL;
    }
  ...
  async function writeConfig(config: Record<string, unknown>): Promise<void> {
    await writeFile(join(tempDir, '.omcp-config.json'), JSON.stringify(config));
  }
  ...
  it('returns frontier default for malformed JSON', async () => {
    await writeFile(join(tempDir, '.omcp-config.json'), 'not-json');
    assert.equal(getModelForMode('team'), DEFAULT_FRONTIER_MODEL);
  });

  it('uses OMCP_DEFAULT_FRONTIER_MODEL when config does not provide a value', () => {
    process.env.OMCP_DEFAULT_FRONTIER_MODEL = 'gpt-5.4-mini';
    assert.equal(getMainDefaultModel(), 'gpt-5.4-mini');
    assert.equal(getModelForMode('team'), 'gpt-5.4-mini');
  });

  it('uses .omcp-config.json env.OMCP_DEFAULT_FRONTIER_MODEL when shell env is absent', async () => {
    await writeConfig({ env: { OMCP_DEFAULT_FRONTIER_MODEL: 'frontier-local' } });
    assert.equal(getMainDefaultModel(), 'frontier-local');
    assert.equal(getModelForMode('team'), 'frontier-local');
  });

  it('uses OMCP_DEFAULT_STANDARD_MODEL when configured in shell env', () => {
    process.env.OMCP_DEFAULT_STANDARD_MODEL = 'gpt-5.4-mini-tuned';
  ...
  it('uses .omcp-config.json env.OMCP_DEFAULT_STANDARD_MODEL when shell env is absent', async () => {
    await writeConfig({ env: { OMCP_DEFAULT_STANDARD_MODEL: 'standard-local' } });
    assert.equal(getEnvConfiguredStandardDefaultModel(), 'standard-local');
    assert.equal(getStandardDefaultModel(), 'standard-local');
  });

  it('prefers shell OMCP_DEFAULT_FRONTIER_MODEL over .omcp-config.json env override', async () => {
    process.env.OMCP_DEFAULT_FRONTIER_MODEL = 'frontier-shell';
    await writeConfig({ env: { OMCP_DEFAULT_FRONTIER_MODEL: 'frontier-local' } });
    assert.equal(getMainDefaultModel(), 'frontier-shell');
  });

  it('keeps explicit config default ahead of OMCP_DEFAULT_FRONTIER_MODEL', async () => {
    process.env.OMCP_DEFAULT_FRONTIER_MODEL = 'gpt-5.4-mini';
    await writeConfig({ models: { default: 'o4-mini' } });
    assert.equal(getModelForMode('team'), 'o4-mini');
  });

  it('keeps explicit mode config ahead of OMCP_DEFAULT_FRONTIER_MODEL', async () => {
    process.env.OMCP_DEFAULT_FRONTIER_MODEL = 'gpt-5.4-mini';
    await writeConfig({ models: { team: 'gpt-4.1', default: 'o4-mini' } });
    assert.equal(getModelForMode('team'), 'gpt-4.1');
  });
  ...
  it('uses OMCP_DEFAULT_SPARK_MODEL when low-complexity config is absent', async () => {
    process.env.OMCP_DEFAULT_SPARK_MODEL = 'gpt-5.3-codex-spark-fast';
    await writeConfig({ models: { team: 'gpt-4.1' } });
  ...
  it('uses .omcp-config.json env.OMCP_DEFAULT_SPARK_MODEL when shell env is absent', async () => {
    await writeConfig({ env: { OMCP_DEFAULT_SPARK_MODEL: 'spark-local' }, models: { team: 'gpt-4.1' } });
    assert.equal(getSparkDefaultModel(), 'spark-local');
  });

  it('falls back to legacy OMCP_SPARK_MODEL when canonical spark env is absent', async () => {
    process.env.OMCP_SPARK_MODEL = 'gpt-5.3-codex-spark-fast';
    await writeConfig({ models: { team: 'gpt-4.1' } });
    assert.equal(getSparkDefaultModel(), 'gpt-5.3-codex-spark-fast');
    assert.equal(getTeamLowComplexityModel(), 'gpt-5.3-codex-spark-fast');
  });

  it('prefers OMCP_DEFAULT_SPARK_MODEL over legacy OMCP_SPARK_MODEL', () => {
    process.env.OMCP_DEFAULT_SPARK_MODEL = 'spark-canonical';
    process.env.OMCP_SPARK_MODEL = 'spark-legacy';
    assert.equal(getSparkDefaultModel(), 'spark-canonical');
  });

  it('reads normalized env overrides from .omcp-config.json', async () => {
    await writeConfig({
      env: {
        OMCP_DEFAULT_FRONTIER_MODEL: ' frontier-local ',
        OMCP_DEFAULT_STANDARD_MODEL: ' standard-local ',
  ...
  it('keeps explicit low-complexity config ahead of OMCP_DEFAULT_SPARK_MODEL', async () => {
    process.env.OMCP_DEFAULT_SPARK_MODEL = 'gpt-5.3-codex-spark-fast';
    await writeConfig({ models: { team_low_complexity: 'gpt-4.1-mini' } });
    assert.equal(getTeamLowComplexityModel(), 'gpt-4.1-mini');
  });

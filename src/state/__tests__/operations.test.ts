...
describe('state operations directory initialization', () => {
  it('creates .omcp/state for state operations without setup', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-test-'));
    try {
  ...
  it('bootstraps tmux-hook from the current tmux pane when available', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-live-'));
    try {
      const tmuxHookConfig = join(wd, '.omcp', 'tmux-hook.json');
  ...
  it('writes and reads deep-interview state', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-readwrite-'));
    try {
  ...
  it('writes and reads autoresearch state', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-autoresearch-'));
    try {
      const writeResponse = await executeStateOperation('state_write', {
        workingDirectory: wd,
        mode: 'autoresearch',
      ...
      assert.deepEqual(writeResponse.payload, {
        success: true,
        mode: 'autoresearch',
        path: join(wd, '.omcp', 'state', 'autoresearch-state.json'),
      });
  ...
  it('creates session-scoped state directory when session_id is provided', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-session-'));
    try {
  ...
  it('serializes concurrent state_write calls per mode file and preserves merged fields', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-concurrency-'));
    try {
  ...
  it('syncs canonical skill-active state for tracked mode writes and clears', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-canonical-'));
    try {
  ...
  it('denies unsupported overlaps without writing the requested mode state', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-deny-overlap-'));
    try {
  ...
  it('does not auto-complete existing workflow state when tracked write validation fails', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-validate-before-transition-'));
    try {
      const sessionDir = join(wd, '.omcp', 'state', 'sessions', 'sess-invalid');
  ...
  it('keeps session-scoped tracked state writable after root-state parse fallback on resume', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-state-ops-resume-root-fallback-'));
    try {
      const stateDir = join(wd, '.omcp', 'state');

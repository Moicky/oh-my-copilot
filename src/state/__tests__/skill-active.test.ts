...
describe('skill-active state helpers', () => {
  it('prefers session-scoped canonical state over root state', async () => {
    await withTempRepo('omcp-skill-active-session-', async (cwd) => {
  ...
  it('drops stale entries from other sessions when syncing canonical state for the current session', async () => {
    await withTempRepo('omcp-skill-active-filter-', async (cwd) => {
      await mkdir(join(cwd, '.omcp', 'state'), { recursive: true });
      await writeSkillActiveStateCopies(cwd, {
        active: true,
      ...
      const rootState = JSON.parse(await readFile(join(cwd, '.omcp', 'state', 'skill-active-state.json'), 'utf-8')) as {
  ...
  it('preserves root-scoped team state when a session-scoped ralph overlap is activated', async () => {
    await withTempRepo('omcp-skill-active-team-ralph-', async (cwd) => {
      await mkdir(join(cwd, '.omcp', 'state'), { recursive: true });
      await writeSkillActiveStateCopies(cwd, {
        active: true,
        skill: 'team',
      ...
      const rootState = JSON.parse(
        await readFile(join(cwd, '.omcp', 'state', 'skill-active-state.json'), 'utf-8'),

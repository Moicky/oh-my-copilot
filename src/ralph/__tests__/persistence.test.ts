...
describe('ensureCanonicalRalphArtifacts', () => {
  it('keeps canonical files authoritative when they already exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ralph-canonical-'));
    try {
      const canonicalPrd = join(cwd, '.omcp', 'plans', 'prd-existing.md');
      const canonicalProgress = join(cwd, '.omcp', 'state', 'ralph-progress.json');
      ...
      await writeFile(join(cwd, '.omcp', 'prd.json'), JSON.stringify({ project: 'legacy-project' }));
      await writeFile(join(cwd, '.omcp', 'progress.txt'), 'legacy line\n');
  ...
  it('migrates legacy PRD/progress files one-way when canonical artifacts are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ralph-migrate-'));
    try {
      const legacyPrdPath = join(cwd, '.omcp', 'prd.json');
      const legacyProgressPath = join(cwd, '.omcp', 'progress.txt');
      ...
      assert.match(canonicalPrd, /Migrated from legacy `.omcp\/prd\.json`/);
      assert.equal(canonicalProgress.source, '.omcp/progress.txt');
  ...
  it('records visual feedback with numeric and qualitative guidance for the next iteration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ralph-visual-feedback-'));
    try {

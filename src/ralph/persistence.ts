...
const LEGACY_PRD_PATH = '.omcp/prd.json';
const LEGACY_PROGRESS_PATH = '.omcp/progress.txt';
...
async function writeMigrationMarker(
  ...
  const markerPath = join(cwd, '.omcp', 'plans', 'ralph-migration-marker.json');
...
async function migrateLegacyPrdIfNeeded(
  ...
  const markdown = [
    `# ${title}`,
    '',
    '> Migrated from legacy `.omcp/prd.json` (read-only compatibility import).',
    '',
    '## Migration Marker',

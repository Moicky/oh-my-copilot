...
describe('ralplan runtime', () => {
  it('persists a successful session-scoped lifecycle through complete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ralplan-runtime-'));
    const sessionId = 'sess-ralplan-success';
    try {
      await mkdir(join(cwd, '.omcp', 'state'), { recursive: true });
      await writeFile(join(cwd, '.omcp', 'state', 'session.json'), JSON.stringify({ session_id: sessionId }));
      ...
      assert.equal(existsSync(join(cwd, '.omcp', 'state', 'ralplan-state.json')), false);
  ...
  it('increments iteration when critic requests a re-review loop', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ralplan-runtime-loop-'));
  ...
  it('marks failed cleanly when execution throws', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ralplan-runtime-fail-'));
  ...
  it('marks cancelled state cleanly', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-ralplan-runtime-cancel-'));

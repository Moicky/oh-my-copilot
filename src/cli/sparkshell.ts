...
const OMCP_SPARKSHELL_BIN_ENV = SPARKSHELL_BIN_ENV_SHARED;

export const SPARKSHELL_USAGE = [
  'Usage: omcp sparkshell <command> [args...]',
  '   or: omcp sparkshell --tmux-pane <pane-id> [--tail-lines <100-1000>]',
  'Runs the native omcp-sparkshell sidecar with direct argv execution or explicit tmux pane summarization.',
...
export function sparkshellBinaryName(platform: NodeJS.Platform = process.platform): string {
  return platform === 'win32' ? 'omcp-sparkshell.exe' : 'omcp-sparkshell';
}
...
export function nestedRepoLocalSparkShellBinaryPath(
  ...
  return join(packageRoot, 'native', 'omcp-sparkshell', 'target', 'release', sparkshellBinaryName(platform));
}
...
export function resolveSparkShellBinaryPath(options: ResolveSparkShellBinaryPathOptions = {}): string {
  ...
  const override = env[OMCP_SPARKSHELL_BIN_ENV]?.trim();
  if (override) {
  ...
  throw new Error(
    `[sparkshell] native binary not found. Checked ${packagedCandidates.join(', ')}, ${repoLocal}, and ${nestedRepoLocal}. `
      + `Set ${OMCP_SPARKSHELL_BIN_ENV} to override the path.`
  );
}
...
export async function resolveSparkShellBinaryPathWithHydration(
  ...
  for (const cached of resolveCachedNativeBinaryCandidatePaths('omcp-sparkshell', version, platform, arch, env, {
  ...
  const hydrated = await hydrateNativeBinary('omcp-sparkshell', { packageRoot, env, platform, arch });
  if (hydrated) return hydrated;

  throw new Error(
    `[sparkshell] native binary not found. Checked cached/native candidates under ${packageRoot}, ${repoLocal}, and ${nestedRepoLocal}. `
      + `Reconnect to the network so OMCP can fetch the release asset, or set ${OMCP_SPARKSHELL_BIN_ENV} to override the path.`
  );
}
...
export async function sparkshellCommand(args: string[]): Promise<void> {
  ...
  const hasExplicitOverride = typeof process.env[OMCP_SPARKSHELL_BIN_ENV] === 'string'
    && process.env[OMCP_SPARKSHELL_BIN_ENV]!.trim().length > 0;

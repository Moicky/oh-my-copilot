...
async function initHooks(): Promise<void> {
  ...
  console.log('Plugins are enabled by default. Disable with OMCP_HOOK_PLUGINS=0.');
}
...
async function statusHooks(): Promise<void> {
  ...
  console.log(
    `Plugins enabled: ${isHookPluginsEnabled(process.env) ? 'yes' : 'no (disabled with OMCP_HOOK_PLUGINS=0)'}`,
  );

...
const AGENTS_USAGE = [
  'Usage:',
  '  omcp agents list [--scope user|project]',
  '  omcp agents add <name> [--scope user|project] [--force]',
...
function inferMutationScope(cwd = process.cwd()): AgentScope {
  const persistedScopePath = join(cwd, '.omcp', 'setup-scope.json');
...
export async function agentsCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      const name = args[1];
      assert.ok(name, 'Usage: omcp agents add <name>');
    ...
    case 'edit': {
      const name = args[1];
      assert.ok(name, 'Usage: omcp agents edit <name>');
    ...
    case 'remove': {
      const name = args[1];
      assert.ok(name, 'Usage: omcp agents remove <name>');

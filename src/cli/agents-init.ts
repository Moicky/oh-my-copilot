...
export const AGENTS_INIT_USAGE = [
  "Usage: omcp agents-init [path] [--dry-run] [--force] [--verbose]",
  "       omcp deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
...
export async function agentsInit(
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active omcp session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by omcp agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active omcp session is running.\n",
  );

...
const HELP = `omcp session - Search prior local session history

Usage:
  omcp session search <query> [options]
...
Examples:
  omcp session search "worker inbox path"
  omcp session search all_workers_idle --since 7d --limit 5
  omcp session search "team api" --project current --json
`;

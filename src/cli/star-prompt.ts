 ...
 * State stored globally (~/.omcp/state/star-prompt.json) so it shows once per user.
 */
...
export function starPromptStatePath(): string {
  return join(homedir(), '.omcp', 'state', 'star-prompt.json');
}
...
export async function markPrompted(): Promise<void> {
  const stateDir = join(homedir(), '.omcp', 'state');
...
export async function maybePromptGithubStar(deps: MaybePromptGithubStarDeps = {}): Promise<void> {
  ...
  const approved = await askYesNoImpl('[omcp] Enjoying oh-my-copilot? Star it on GitHub? [Y/n] ');
  if (!approved) return;
  ...
  if (star.ok) {
    ...
    log('[omcp] Thanks for the star!');
    return;
  }
  const warn = deps.warnFn ?? console.warn;
  warn(`[omcp] Could not star repository automatically: ${star.error}`);
}

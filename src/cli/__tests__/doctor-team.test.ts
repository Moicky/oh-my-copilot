...
describe('omcp doctor --team', () => {
  it('exits non-zero and prints resume_blocker when team state references missing tmux session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-doctor-team-'));
    try {
      const teamRoot = join(wd, '.omcp', 'state', 'team', 'alpha');
  ...
  it('prints slow_shutdown when shutdown request is stale and ack missing', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-doctor-team-'));
    try {
      const workerDir = join(wd, '.omcp', 'state', 'team', 'beta', 'workers', 'worker-1');
      await mkdir(workerDir, { recursive: true });
      await writeFile(join(wd, '.omcp', 'state', 'team', 'beta', 'config.json'), JSON.stringify({
        name: 'beta',
  ...
  it('prints delayed_status_lag when worker is working and heartbeat is stale', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-doctor-team-'));
    try {
      const workerDir = join(wd, '.omcp', 'state', 'team', 'gamma', 'workers', 'worker-1');
      await mkdir(workerDir, { recursive: true });
      await writeFile(join(wd, '.omcp', 'state', 'team', 'gamma', 'config.json'), JSON.stringify({
        name: 'gamma',
  ...
  it('prints stale_leader when HUD state is old and team tmux session is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-doctor-team-'));
    try {
      const teamRoot = join(wd, '.omcp', 'state', 'team', 'epsilon');
      await mkdir(join(teamRoot, 'workers', 'worker-1'), { recursive: true });
      await writeFile(join(teamRoot, 'config.json'), JSON.stringify({
        name: 'epsilon',
        tmux_session: 'omcp-team-epsilon',
      }));
  ...
  it('does not emit stale_leader when HUD state is fresh', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omcp-doctor-team-'));
    try {
      const teamRoot = join(wd, '.omcp', 'state', 'team', 'zeta');
      await mkdir(join(teamRoot, 'workers', 'worker-1'), { recursive: true });
      await writeFile(join(teamRoot, 'config.json'), JSON.stringify({
        name: 'zeta',
        tmux_session: 'omcp-team-zeta',
      }));

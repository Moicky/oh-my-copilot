...
describe("omcp setup refresh summary and dry-run behavior", () => {
  ...
  it("creates .gitignore with OMCP project ignore rules during project-scoped setup", async () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(wd, ".omcp", "state")), true);
      assert.equal(
  ...
  it("ignores legacy ~/.omc/mcp-registry.json during setup unless candidates are passed explicitly", async () => {
    ...
    try {
      ...
      assert.match(output, /move it to .*\.omcp\/mcp-registry\.json/i);
    } finally {
      if (typeof previousHome === "string") process.env.HOME = previousHome;
      else delete process.env.HOME;
      if (typeof previousCodexHome === "string") process.env.CODEX_HOME = previousCodexHome;

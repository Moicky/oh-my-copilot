...
describe('package bin contract', () => {
  it('declares omcp with an explicit relative bin path and avoids packaging platform-specific native binaries', () => {
    ...
    assert.equal(pkg.scripts?.['build:explore'], 'cargo build -p omcp-explore-harness');
    ...
    assert.equal(pkg.scripts?.['test:explore'], 'cargo test -p omcp-explore-harness && node --test dist/cli/__tests__/explore.test.js dist/hooks/__tests__/explore-routing.test.js dist/hooks/__tests__/explore-sparkshell-guidance-contract.test.js');
    ...
    const binPath = join(process.cwd(), 'dist', 'cli', 'omcp.js');
    ...
    const binEntry = results[0]?.files?.find((file) => file.path === 'dist/cli/omcp.js');
    assert.ok(binEntry, 'expected npm pack output to include dist/cli/omcp.js');

    const packagedHarnessPath = process.platform === 'win32' ? 'bin/omcp-explore-harness.exe' : 'bin/omcp-explore-harness';
    const packagedHarnessEntry = results[0]?.files?.find((file) => file.path === packagedHarnessPath);
    const packagedHarnessMetaEntry = results[0]?.files?.find((file) => file.path === 'bin/omcp-explore-harness.meta.json');
    ...
    const crateManifestEntry = results[0]?.files?.find((file) => file.path === 'crates/omcp-explore/Cargo.toml');
    const crateMainEntry = results[0]?.files?.find((file) => file.path === 'crates/omcp-explore/src/main.rs');
    ...
    assert.ok(crateManifestEntry, 'expected npm pack output to include crates/omcp-explore/Cargo.toml');
    assert.ok(crateMainEntry, 'expected npm pack output to include crates/omcp-explore/src/main.rs');
  });
});

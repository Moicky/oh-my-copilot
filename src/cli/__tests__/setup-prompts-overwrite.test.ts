...
describe('omcp setup prompt/native-agent overwrite behavior', () => {
  ...
  it('installs only active/internal catalog prompts and native agents', async () => {
    ...
    try {
      ...
      const promptsDir = join(wd, '.copilot', 'prompts');
      const nativeAgentsDir = join(wd, '.copilot', 'agents');
  ...
  it('removes stale merged/unlisted prompts on --force', async () => {
    ...
    try {
      ...
      for (const stalePrompt of stalePrompts) {
        const stalePath = join(wd, '.copilot', 'prompts', stalePrompt);
      ...
      for (const stalePrompt of stalePrompts) {
        assert.equal(existsSync(join(wd, '.copilot', 'prompts', stalePrompt)), false);
      }
      assert.equal(existsSync(join(wd, '.copilot', 'prompts', 'executor.md')), true);
    } finally {
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('removes stale native agents with the obsolete bridge field during normal setup', async () => {
    ...
    try {
      ...
      const stalePath = join(wd, '.copilot', 'agents', 'legacy-skill-agent.toml');
      await writeFile(
        stalePath,
        [

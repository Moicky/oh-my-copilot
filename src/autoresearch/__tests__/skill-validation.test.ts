...
describe('autoresearch skill validation', () => {
  ...
  it('treats missing validation mode as incomplete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-autoresearch-skill-validation-'));
    try {
  ...
  it('requires a validator pass artifact for mission-validator-script mode', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-autoresearch-validator-script-'));
    try {
      const artifactPath = join(cwd, '.omcp', 'specs', 'autoresearch-demo', 'completion.json');
      await mkdir(join(cwd, '.omcp', 'specs', 'autoresearch-demo'), { recursive: true });
  ...
  it('requires prompt, architect approval, and output artifact for prompt-architect-artifact mode', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omcp-autoresearch-architect-validation-'));
    try {
      const artifactDir = join(cwd, '.omcp', 'specs', 'autoresearch-demo');

...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omcp-autoresearch-runtime-'));
...
describe('autoresearch runtime', () => {
  ...
  it('allows untracked .omcp runtime files when checking reset safety', async () => {
    const repo = await initRepo();
    try {
  ...
  it('prepares runtime artifacts and persists autoresearch mode state', async () => {
    ...
    try {
      ...
      const worktreePath = join(repo, '.omcp', 'worktrees', 'autoresearch-missions-demo-20260314t000000z');
...
describe('autoresearch parity decisions', () => {
  it('keeps improved candidates and resets discarded candidates back to the last kept commit', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.omcp', 'worktrees', 'autoresearch-missions-demo-20260314t010000z');

...
const FOUNDATION_CAPABILITIES: AdaptCapabilityReport[] = [
	...
	capability(
		...
		"shared-contract",
		"ready",
		"Envelope output links canonical OMCP PRD/test-spec artifacts when they exist.",
	),
	capability(
...
const TARGET_DESCRIPTORS: Record<AdaptTarget, AdaptTargetDescriptor> = {
	openclaw: {
		...
		summary:
			"OMCP-owned adapter around existing OpenClaw notification, gateway, and lifecycle observation surfaces.",
		followupHint:
		...
		capabilities: [
			...
			capability(
				...
				"shared-contract",
				"ready",
				"Probe, status, and envelope surface the existing OMCP to OpenClaw lifecycle mapping without claiming remote execution health.",
			),
		],
	},
	hermes: {
		...
		capabilities: [
			...
			capability(
				...
				"shared-contract",
				"stub",
				"Hermes envelope/bootstrap metadata maps OMCP lifecycle intent into ACP and gateway guidance without claiming deep control.",
			),
		],
	},
};

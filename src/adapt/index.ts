...
export function buildAdaptPlanningLink(cwd: string): AdaptPlanningLink {
	...
	if (!selection.prdPath) {
		return {
			...
			summary:
				"No canonical OMCP PRD/test-spec artifacts are present in this worktree.",
		};
	}
...
export function buildAdaptStatusReport(
	...
	return {
		...
		phase: "foundation",
		summary: initialized
			? `${descriptor.displayName} adapter foundation is initialized under OMCP-owned paths.`
...
export function buildAdaptDoctorReport(
	...
	if (!planning.prdPath) {
		issues.push({
			...
			message:
				"No canonical OMCP PRD artifact is available to link into the adapter envelope.",
		});
	}
...
export async function buildAdaptDoctorReportForTarget(
	...
	return {
		...report,
		summary:
			"Hermes doctor inspects external ACP, gateway, and session-store evidence plus OMCP-owned adapter readiness.",
		issues,
		nextSteps: [
			...bootstrap.nextSteps,
...
export function initAdaptFoundation(
	...
	return {
		...
		write,
		summary: write
			? `${descriptor.displayName} adapter foundation was written under OMCP-owned paths.`
...
export async function initAdaptFoundationForTarget(
	...
	return {
		...
		envelope,
		summary: write
			? "Hermes adapter metadata was written under OMCP-owned paths with external runtime evidence."

...
const OMCP_METADATA_SEGMENT_RE = /^\[OMCP(?:[#\]].*)?$/;
...
function isMetadataOnlyTmuxSegment(segment: string): boolean {
  return OMCP_METADATA_SEGMENT_RE.test(segment)
...
function isMetadataOnlyTmuxLine(line: string): boolean {
  ...
  const hasExplicitStatusSegment = segments.some((segment) => OMCP_METADATA_SEGMENT_RE.test(segment) || HUD_STATUS_SEGMENT_RE.test(segment));
...
function shouldUsePidFallback(): boolean {
  return process.env.OMCP_TMUX_PID_FALLBACK === "1";
}

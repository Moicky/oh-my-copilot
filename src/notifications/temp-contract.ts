export const OMCP_NOTIFY_TEMP_ENV = 'OMCP_NOTIFY_TEMP';
export const OMCP_NOTIFY_TEMP_CONTRACT_ENV = 'OMCP_NOTIFY_TEMP_CONTRACT';
...
export function parseNotifyTempContractFromArgs(
  ...
  const envActivated = env[OMCP_NOTIFY_TEMP_ENV] === '1';
...
export function isNotifyTempEnvActive(env: NodeJS.ProcessEnv = process.env): boolean {
  return env[OMCP_NOTIFY_TEMP_ENV] === '1';
}

export function readNotifyTempContractFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): NotifyTempContract | null {
  const raw = env[OMCP_NOTIFY_TEMP_CONTRACT_ENV];
  if (!raw) return null;
  try {

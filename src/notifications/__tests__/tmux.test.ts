...
describe('getCurrentTmuxSession', () => {
  ...
  const originalPidFallback = process.env.OMCP_TMUX_PID_FALLBACK;
  ...
  afterEach(() => {
    ...
    if (originalPidFallback !== undefined) {
      process.env.OMCP_TMUX_PID_FALLBACK = originalPidFallback;
    } else {
      delete process.env.OMCP_TMUX_PID_FALLBACK;
    }
  });
  ...
  it('skips ps-based pid fallback on native Windows', () => {
    ...
    delete process.env.TMUX;
    process.env.OMCP_TMUX_PID_FALLBACK = '1';
    try {

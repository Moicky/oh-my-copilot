...
describe('getVerbosity', () => {
  const origEnv = process.env.OMCP_NOTIFY_VERBOSITY;

  afterEach(() => {
    if (origEnv === undefined) {
      delete process.env.OMCP_NOTIFY_VERBOSITY;
    } else {
      process.env.OMCP_NOTIFY_VERBOSITY = origEnv;
    }
  });

...
const ENV_KEYS = [
  ...
  'OMCP_DISCORD_MENTION',
  'OMCP_TELEGRAM_BOT_TOKEN',
  'OMCP_TELEGRAM_NOTIFIER_BOT_TOKEN',
  'OMCP_TELEGRAM_CHAT_ID',
  'OMCP_TELEGRAM_NOTIFIER_CHAT_ID',
  'OMCP_TELEGRAM_NOTIFIER_UID',
  'OMCP_SLACK_WEBHOOK_URL',
  'OMCP_SLACK_MENTION',
  'OMCP_REPLY_ENABLED',
  'OMCP_REPLY_DISCORD_USER_IDS',
  'OMCP_REPLY_POLL_INTERVAL_MS',
...
describe('buildConfigFromEnv', () => {
  ...
  it('builds discord-bot config from env vars', () => {
    process.env.OMCP_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMCP_DISCORD_NOTIFIER_CHANNEL = '123456';
  ...
  it('includes validated mention in discord-bot config', () => {
    process.env.OMCP_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMCP_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.OMCP_DISCORD_MENTION = '<@12345678901234567>';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('rejects invalid mention in env var', () => {
    process.env.OMCP_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMCP_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.OMCP_DISCORD_MENTION = '@everyone';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds discord webhook config from env var', () => {
    process.env.OMCP_DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds telegram config from env vars', () => {
    process.env.OMCP_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.OMCP_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds slack config from env var', () => {
    process.env.OMCP_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('includes a validated slack mention in slack config', () => {
    process.env.OMCP_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    process.env.OMCP_SLACK_MENTION = '<!here>';
  ...
  it('uses OMCP_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback', () => {
    process.env.OMCP_TELEGRAM_NOTIFIER_BOT_TOKEN = '123:fallback';
    process.env.OMCP_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses OMCP_TELEGRAM_NOTIFIER_UID as fallback for chat ID', () => {
    process.env.OMCP_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.OMCP_TELEGRAM_NOTIFIER_UID = 'uid-999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds config with multiple platforms from env', () => {
    process.env.OMCP_DISCORD_NOTIFIER_BOT_TOKEN = 'bot-token';
    process.env.OMCP_DISCORD_NOTIFIER_CHANNEL = 'channel-123';
    process.env.OMCP_TELEGRAM_BOT_TOKEN = '456:tg';
    process.env.OMCP_TELEGRAM_CHAT_ID = 'chat-789';
    process.env.OMCP_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';

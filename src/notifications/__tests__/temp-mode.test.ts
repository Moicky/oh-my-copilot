...
describe('notification temp mode', () => {
  ...
  it('temp contract bypasses persistent file/profile routing', async () => {
    ...
    process.env.OMCP_NOTIFY_PROFILE = 'file-profile';
    process.env.OMCP_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/temp-only';
    process.env.OMCP_NOTIFY_TEMP_CONTRACT = JSON.stringify({
      active: true,
      selectors: ['slack'],

...
function commitHygieneReportsDir(cwd: string): string {
  return join(resolve(cwd), '.omcp', 'reports', 'team-commit-hygiene')
}
...
export function buildTeamCommitHygieneContext(params: {
  ...
  const recommendedNextSteps = [
    ...
    'Derive semantic commit boundaries from completed task subjects, code diffs, and shutdown reports rather than from omcp(team) operational commit subjects.',
...
export function renderTeamCommitHygieneMarkdown(context: TeamCommitHygieneContext): string {
  return [
    ...
    '',
    '1. Treat `omcp(team): ...` runtime commits as temporary scaffolding, not as the final PR history.',

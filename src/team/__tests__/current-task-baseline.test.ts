...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omcp-current-task-baseline-'));
...
describe('current-task-baseline', () => {
  it('records a launch worktree branch baseline on successful ensureWorktree', async () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(repo, '.omcp', 'state', 'current-task-baseline.json')), true);
    } finally {
  ...
  it('updates branch lifecycle metadata when PR info is observed', async () => {
    ...
    try {
      ...
      const raw = JSON.parse(readFileSync(join(repo, '.omcp', 'state', 'current-task-baseline.json'), 'utf-8'));

...
describe("worker bootstrap", () => {
  it("worker skill lifecycle instructions are claim-safe (issue #448)", async () => {
    ...
    assert.match(workerSkill, /omcp team api claim-task/);
    assert.match(workerSkill, /omcp team api transition-task-status/);
  ...
  it("generateWorkerOverlay includes the team name", () => {
    ...
    assert.match(overlay, /omcp team api claim-task/);
    assert.match(overlay, /omcp team api transition-task-status/);
    assert.match(overlay, /omcp team api release-task-claim/);
    assert.doesNotMatch(
      overlay,
  ...
  it("applyWorkerOverlay reaps stale AGENTS lock directory", async () => {
    ...
    try {
      const agentsMdPath = join(cwd, "AGENTS.md");
      const lockPath = join(cwd, ".omcp", "state", "agents-md.lock");
  ...
  it("generateInitialInbox includes worker name, team name, and all tasks", () => {
    ...
    assert.match(inbox, /omcp team api claim-task/);
    assert.match(inbox, /omcp team api transition-task-status/);
  ...
  it("generateTriggerMessage does not contain [OMCP_TMUX_INJECT]", () => {
    const message = generateTriggerMessage("worker-1", "team-safe");
    assert.equal(message.includes("[OMCP_TMUX_INJECT]"), false);
  });
  ...
  it("generateTriggerMessage contains the inbox path", () => {
    ...
    assert.match(
      message,
      /\.omcp\/state\/team\/team-path\/workers\/worker-9\/inbox\.md/,
    );
  ...
  it("buildTriggerDirective keeps human text separate from orchestration intent", () => {
    ...
    assert.match(directive.text, /\.omcp\/state\/team\/team-path\/workers\/worker-9\/inbox\.md/);
    assert.doesNotMatch(directive.text, /OMCP_INTENT/);
  });
  ...
  it("generateTriggerMessage uses provided state-root reference for worktree workers", () => {
    ...
    assert.match(
      message,
      /\$OMCP_TEAM_STATE_ROOT\/team\/team-path\/workers\/worker-9\/inbox\.md/,
    );
  ...
  it("generateMailboxTriggerMessage contains mailbox path and count", () => {
    ...
    assert.match(
      message,
      /Read .*\.omcp\/state\/team\/team-mail\/mailbox\/worker-2\.json/,
    );
  ...
  it("generateMailboxTriggerMessage uses provided state-root reference for worktree workers", () => {
    ...
    assert.match(
      message,
      /read .*\$OMCP_TEAM_STATE_ROOT\/team\/team-mail\/mailbox\/worker-2\.json/i,
    );
  ...
  it("generateLeaderMailboxTriggerMessage tells the leader to read the mailbox and decide the next step", () => {
    ...
    assert.match(
      message,
      /Read .*\.omcp\/state\/team\/team-mail\/mailbox\/leader-fixed\.json/,
    );
  ...
  it("generateLeaderMailboxTriggerMessage uses provided state-root reference for worktree leaders", () => {
    ...
    assert.match(
      message,
      /read .*\$OMCP_TEAM_STATE_ROOT\/team\/team-mail\/mailbox\/leader-fixed\.json/i,
    );
  ...
  it("generateWorkerRootAgentsContent includes hardcoded paths and role prompt without base AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      leaderCwd: "/repo",
      worktreePath: "/repo/.omcp/team/root-team/worktrees/worker-3",
    });
  ...
  it("generateWorkerRootAgentsContent hardcodes runtime paths and role prompt without inherited AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      workerRole: "writer",
      rolePromptContent: "<identity>You are Writer.</identity>",
      teamStateRoot: "/tmp/project/.omcp/state",
      leaderCwd: "/tmp/project",
      worktreePath: "/tmp/project/.omcp/team/root-team/worktrees/worker-2",
    });

    assert.match(content, /# Team Worker Runtime Instructions/);
    assert.match(content, /Inbox path: \/tmp\/project\/.omcp\/state\/team\/root-team\/workers\/worker-2\/inbox\.md/);
    assert.match(content, /Mailbox path: \/tmp\/project\/.omcp\/state\/team\/root-team\/mailbox\/worker-2\.json/);
    assert.match(content, /Leader mailbox path: \/tmp\/project\/.omcp\/state\/team\/root-team\/mailbox\/leader-fixed\.json/);
  ...
  it("writeWorkerWorktreeRootAgentsFile writes disposable root AGENTS and remove restores tracked content", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omcp-worker-root-agents-"));
    const worktree = join(cwd, "worktree");
    try {
      await mkdir(join(cwd, ".omcp", "state", "team", "restore-team", "workers", "worker-1"), { recursive: true });
      ...
      const outPath = await writeWorkerWorktreeRootAgentsFile({
        ...
        teamStateRoot: join(cwd, ".omcp", "state"),
        leaderCwd: cwd,
      ...
      await removeWorkerWorktreeRootAgentsFile("restore-team", "worker-1", join(cwd, ".omcp", "state"), worktree);

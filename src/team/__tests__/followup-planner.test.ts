...
describe('followup-planner', () => {
  it('resolves available agent types from explicit prompt directories', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'omcp-followup-roster-'));
    try {
  ...
  it('builds concrete team staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omcp team 3:executor "Fix flaky integration tests and update README"');
  ...
  it('builds concrete ralph staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omcp ralph "Investigate auth regression and verify the fix"');

...
async function readTeamNudgeProgressMs(cwd: string, teamName: string): Promise<number> {
  const nudgeState = await readJsonIfExists(join(cwd, '.omcp', 'state', 'team-leader-nudge.json'));
...
async function readCurrentTaskBaselineMs(worktreePath: string): Promise<number> {
  return await statMsIfExists(join(worktreePath, '.omcp', 'state', 'current-task-baseline.json'));
}

...
const INJECTION_MARKER = '[OMCP_TMUX_INJECT]';
const MODEL_INSTRUCTIONS_FILE_KEY = 'model_instructions_file';
const OMCP_BYPASS_DEFAULT_SYSTEM_PROMPT_ENV = 'OMCP_BYPASS_DEFAULT_SYSTEM_PROMPT';
const OMCP_MODEL_INSTRUCTIONS_FILE_ENV = 'OMCP_MODEL_INSTRUCTIONS_FILE';
const OMCP_TEAM_WORKER_CLI_ENV = 'OMCP_TEAM_WORKER_CLI';
const OMCP_TEAM_WORKER_CLI_MAP_ENV = 'OMCP_TEAM_WORKER_CLI_MAP';
const OMCP_TEAM_WORKER_LAUNCH_MODE_ENV = 'OMCP_TEAM_WORKER_LAUNCH_MODE';
const OMCP_TEAM_AUTO_INTERRUPT_RETRY_ENV = 'OMCP_TEAM_AUTO_INTERRUPT_RETRY';
const GEMINI_PROMPT_INTERACTIVE_FLAG = '-i';
const GEMINI_APPROVAL_MODE_FLAG = '--approval-mode';
const GEMINI_APPROVAL_MODE_YOLO = 'yolo';
const OMCP_LEADER_NODE_PATH_ENV = 'OMCP_LEADER_NODE_PATH';
const OMCP_LEADER_CLI_PATH_ENV = 'OMCP_LEADER_CLI_PATH';
...
function isHudWatchPane(pane: TmuxPaneInfo): boolean {
  ...
  return /\bomcp\b.*\bhud\b.*--watch/i.test(start);
}
...
export function resolveTeamWorkerLaunchMode(
  ...
  const raw = String(env[OMCP_TEAM_WORKER_LAUNCH_MODE_ENV] ?? 'interactive').trim().toLowerCase();
  if (raw === '' || raw === 'interactive') return 'interactive';
  if (raw === 'prompt') return 'prompt';
  throw new Error(`Invalid ${OMCP_TEAM_WORKER_LAUNCH_MODE_ENV} value "${env[OMCP_TEAM_WORKER_LAUNCH_MODE_ENV]}". Expected: interactive, prompt`);
}
...
export function resolveTeamWorkerCli(launchArgs: string[] = [], env: NodeJS.ProcessEnv = process.env): TeamWorkerCli {
  const mode = normalizeTeamWorkerCliMode(env[OMCP_TEAM_WORKER_CLI_ENV]);
...
export function resolveTeamWorkerCliPlan(
  ...
  const rawMap = String(env[OMCP_TEAM_WORKER_CLI_MAP_ENV] ?? '').trim();
  ...
  if (entries.length !== 1 && entries.length !== workerCount) {
    throw new Error(
      `Invalid ${OMCP_TEAM_WORKER_CLI_MAP_ENV} length ${entries.length}; `
  ...
  return expanded.map((entry) => {
    const mode = normalizeTeamWorkerCliMode(entry, OMCP_TEAM_WORKER_CLI_MAP_ENV);
...
function resolveLeaderNodePath(): string {
  const envOverride = process.env[OMCP_LEADER_NODE_PATH_ENV];
...
function shouldBypassDefaultSystemPrompt(env: NodeJS.ProcessEnv): boolean {
  return env[OMCP_BYPASS_DEFAULT_SYSTEM_PROMPT_ENV] !== '0';
}

function buildModelInstructionsOverride(cwd: string, env: NodeJS.ProcessEnv): string {
  const filePath = translatePathForMsys(env[OMCP_MODEL_INSTRUCTIONS_FILE_ENV] || join(cwd, 'AGENTS.md'));
...
export function buildWorkerStartupCommand(
  ...
  const resolvedLeaderNodePath = processSpec.env[OMCP_LEADER_NODE_PATH_ENV]?.trim() || resolveLeaderNodePath();
...
export function createTeamSession(
  ...
  try {
    ...
    const omcpEntry = resolveOmcpCliEntryPath();
    if (omcpEntry && omcpEntry.trim() !== '') {
      const hudCmd = `node ${shellQuoteSingle(translatePathForMsys(omcpEntry))} hud --watch`;
...
export function restoreStandaloneHudPane(
  ...
  const omcpEntry = resolveOmcpCliEntryPath();
  if (!omcpEntry || omcpEntry.trim() === '') return null;

  const hudCmd = `${shellQuoteSingle(translatePathForMsys(resolveLeaderNodePath()))} ${shellQuoteSingle(translatePathForMsys(omcpEntry))} hud --watch`;
  const hudCwd = translatePathForMsys(cwd);
  const hudResult = runTmux([
    'split-window',
    '-v',
...
function dismissClaudeBypassPermissionsPromptIfPresent(target: string, captured: string): boolean {
  if (process.env.OMCP_TEAM_AUTO_ACCEPT_BYPASS === '0') return false;
...
function resolveSendStrategyFromEnv(): 'auto' | 'queue' | 'interrupt' {
  const raw = String(process.env.OMCP_TEAM_SEND_STRATEGY || '')
    .trim()
    .toLowerCase();
...
function resolveWorkerCliFromMapForSend(
  ...
  try {
    const mode = normalizeTeamWorkerCliMode(selectedRaw, OMCP_TEAM_WORKER_CLI_MAP_ENV);
 ...
 * 2) per-worker OMCP_TEAM_WORKER_CLI_MAP entry (worker index aware)
 * 3) global/default OMCP_TEAM_WORKER_CLI behavior
 */
...
export function waitForWorkerReady(
  ...
  const check = (): boolean => {
    ...
    if (paneHasTrustPrompt(result.stdout)) {
      ...
      // Opt-out by setting OMCP_TEAM_AUTO_TRUST=0.
      if (process.env.OMCP_TEAM_AUTO_TRUST !== '0') {
        sendRobustEnter();
        promptDismissed = true;
        return false;
      }
...
export function dismissTrustPromptIfPresent(
  ...
  if (process.env.OMCP_TEAM_AUTO_TRUST === '0') return false;
...
export async function sendToWorker(
  ...
  const allowAutoInterruptRetry = process.env[OMCP_TEAM_AUTO_INTERRUPT_RETRY_ENV] !== '0';

...
import { copilotPromptsDir, packageRoot } from '../utils/paths.js';
...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `omcp team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
  ...
  return {
    shellCommand: `omcp ralph ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `omcp team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.',

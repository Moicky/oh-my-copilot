...
const OMCP_DISPATCH_LOCK_TIMEOUT_ENV = 'OMCP_DISPATCH_LOCK_TIMEOUT_MS';
...
export function resolveDispatchLockTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env[OMCP_DISPATCH_LOCK_TIMEOUT_ENV];
    ...
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          ...
          `Set ${OMCP_DISPATCH_LOCK_TIMEOUT_ENV} to increase (current: ${timeoutMs}ms, max: ${MAX_DISPATCH_LOCK_TIMEOUT_MS}ms).`
        );
      }

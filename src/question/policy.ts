...
export async function evaluateQuestionPolicy(
  ...
  if (hasWorkerContext(env)) {
    return {
      ...
      sessionId,
      code: 'worker_blocked',
      message: 'omcp question is unavailable for OMCP team workers; only non-team leader sessions may ask user questions.',
  ...
  if (activeTeams.length > 0) {
    ...
    return {
      ...
      sessionId,
      code: 'team_blocked',
      message: `omcp question is unavailable while this session owns active team mode: ${summary}.`,
      fallbackAllowed: false,
      activeModes,
      activeSkills,
      activeTeams,
  ...
  if (blocked.length > 0) {
    return {
      ...
      message: `omcp question is unavailable while auto-executing workflows are active: ${blocked.join(', ')}.`,
      fallbackAllowed: false,
      activeModes,
      activeSkills,
      activeTeams,

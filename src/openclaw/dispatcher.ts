 ...
 *
 * SECURITY: Command gateway requires OMCP_OPENCLAW_COMMAND=1 opt-in.
 ...
 * gateway timeout > OMCP_OPENCLAW_COMMAND_TIMEOUT_MS > default.
 */
export function resolveCommandTimeoutMs(
  gatewayTimeout?: number,
  envTimeoutRaw: string | undefined = process.env.OMCP_OPENCLAW_COMMAND_TIMEOUT_MS,
): number {
 ...
 * - Requires OMCP_OPENCLAW_COMMAND=1 opt-in (separate gate from OMCP_OPENCLAW)
 * - Timeout is configurable via gateway.timeout or OMCP_OPENCLAW_COMMAND_TIMEOUT_MS
...
export async function wakeCommandGateway(
  ...
  if (process.env.OMCP_OPENCLAW_COMMAND !== "1") {
    return {
      gateway: gatewayName,
      success: false,
      error: "Command gateway disabled (set OMCP_OPENCLAW_COMMAND=1 to enable)",
    };
  }

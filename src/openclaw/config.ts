 ...
 * Config file path can be overridden via OMCP_OPENCLAW_CONFIG env var (points to a separate file).
 */
...
export function inspectOpenClawConfig(
	...
	if (!activationGateEnabled) {
		return {
			...
			warnings: [],
			detail: "OpenClaw is disabled locally because OMCP_OPENCLAW=1 is not set.",
			config: null,
		};
	}
	...
	if (!configExists) {
		return {
			...
			warnings: [],
			detail: envOverride
				? `OMCP_OPENCLAW_CONFIG points to ${configPath}, but the file does not exist.`
		...
		if (envOverride) {
			...
			if (!isValidOpenClawConfig(raw)) {
				return {
					...
					warnings: [],
					detail:
						"OMCP_OPENCLAW_CONFIG is present but does not contain a valid OpenClaw config.",
					config: null,
				};
			}
			...
			return {
				...
				explicitOverridesAliases: false,
				warnings: [],
				detail: "OpenClaw config loaded from OMCP_OPENCLAW_CONFIG.",
				config: raw,
			};
		}
 ...
 *
 * Returns null when:
 * - OMCP_OPENCLAW env var is not "1"
...
export function getOpenClawConfig(): OpenClawConfig | null {
	// Activation gate: only active when OMCP_OPENCLAW=1
	if (process.env.OMCP_OPENCLAW !== "1") {
		return null;
	}
	...
	try {
		const envOverride = process.env.OMCP_OPENCLAW_CONFIG;

		if (envOverride) {
			// OMCP_OPENCLAW_CONFIG points to a separate config file

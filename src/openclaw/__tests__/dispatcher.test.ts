...
describe('resolveCommandTimeoutMs', () => {
  afterEach(() => {
    delete process.env.OMCP_OPENCLAW_COMMAND_TIMEOUT_MS;
  });
...
describe('wakeCommandGateway - command gate', () => {
  ...
  it('returns error when OMCP_OPENCLAW_COMMAND is not set', async () => {
    ...
    assert.ok(result.error?.includes('OMCP_OPENCLAW_COMMAND'));
  });

  it('succeeds when OMCP_OPENCLAW_COMMAND=1 and command exits 0', async () => {
    const { wakeCommandGateway } = await import('../dispatcher.js');
    process.env.OMCP_OPENCLAW_COMMAND = '1';
  ...
  it('uses env timeout when gateway timeout is not set', async () => {
    const { wakeCommandGateway } = await import('../dispatcher.js');
    process.env.OMCP_OPENCLAW_COMMAND = '1';
    process.env.OMCP_OPENCLAW_COMMAND_TIMEOUT_MS = '100';

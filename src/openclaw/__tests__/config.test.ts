...
describe("getOpenClawConfig", () => {
	...
	beforeEach(() => {
		...
		tmpDir = join(tmpdir(), `omcp-openclaw-test-${Date.now()}`);
		mkdirSync(tmpDir, { recursive: true });
		process.env.COPILOT_HOME = join(tmpDir, ".copilot");
	});
	...
	it("returns null when OMCP_OPENCLAW is not set", async () => {
	...
	it("returns null when OMCP_OPENCLAW !== '1'", async () => {
		process.env.OMCP_OPENCLAW = "0";
		const { getOpenClawConfig, resetOpenClawConfigCache } = await import(
			"../config.js"
		);
		resetOpenClawConfigCache();
	...
	it("returns null when OMCP_OPENCLAW_CONFIG file does not exist", async () => {
		process.env.OMCP_OPENCLAW = "1";
		process.env.OMCP_OPENCLAW_CONFIG = join(tmpDir, "nonexistent.json");
		const { getOpenClawConfig, resetOpenClawConfigCache } = await import(
			"../config.js"
		);
		resetOpenClawConfigCache();
	...
	it("reads config from OMCP_OPENCLAW_CONFIG override file", async () => {
		process.env.OMCP_OPENCLAW = "1";
		...
		process.env.OMCP_OPENCLAW_CONFIG = configPath;
		const { getOpenClawConfig, resetOpenClawConfigCache } = await import(
			"../config.js"
		);
		resetOpenClawConfigCache();
...
describe("getOpenClawConfig generic alias normalization", () => {
	...
	beforeEach(() => {
		originalEnv = { ...process.env };
		tmpDir = join(tmpdir(), `omcp-openclaw-alias-test-${Date.now()}`);
		mkdirSync(tmpDir, { recursive: true });
		process.env.OMCP_OPENCLAW = "1";
	...
	it("normalizes custom_webhook_command alias to openclaw runtime config", async () => {
		const omcpConfigPath = join(tmpDir, ".copilot", ".omcp-config.json");
...
describe("inspectOpenClawConfig", () => {
	...
	beforeEach(() => {
		originalEnv = { ...process.env };
		tmpDir = join(tmpdir(), `omcp-openclaw-inspect-test-${Date.now()}`);
		mkdirSync(tmpDir, { recursive: true });
		process.env.HOME = tmpDir;
		process.env.COPILOT_HOME = join(tmpDir, ".copilot");
		delete process.env.OMCP_OPENCLAW_CONFIG;
	...
	it("reports disabled state when OMCP_OPENCLAW is not enabled", async () => {
		delete process.env.OMCP_OPENCLAW;
